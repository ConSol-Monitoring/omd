...
#grafana interface
TestUtils::test_command({ cmd => "/bin/su - $site -c 'lib/monitoring-plugins/check_http -t 60 -H 127.0.0.1 -u /$site/grafana/ -p 8003 -k \"X-WEBAUTH-USER: omdadmin\" -s \"<title>Grafana</title>\"'", like => '/HTTP OK:/', waitfor => 'HTTP\ OK:', maxwait => 180 });
TestUtils::test_command({ cmd => "/omd/sites/$site/lib/monitoring-plugins/check_http -t 60 -H localhost -a omdadmin:omd -u '/$site/grafana/' -s '\"login\":\"omdadmin\"'", like => '/HTTP OK:/', waitfor => 'HTTP\ OK:', maxwait => 180 });
...
TestUtils::test_command({ cmd => "/bin/su - $site -c 'cat var/log/grafana/grafana.log'", like => '/HTTP Server Listen/', waitfor => 'HTTP\ Server\ Listen', maxwait => 180 });
TestUtils::test_command({ cmd => "/omd/sites/$site/lib/monitoring-plugins/check_http -t 60 -H localhost -S -a omdadmin:omd -u '/$site/grafana/' -s '\"login\":\"omdadmin\"'", like => '/HTTP OK:/', waitfor => 'HTTP\ OK:', maxwait => 180 });
...
# then test external ip and make sure it doesn't work
...
{
    my $test = {
        cmd  => "/bin/su - $site -c 'grep metrics_path: share/grafana/01-grafana.yml'",
        like => ['/metrics/'],
    };
    TestUtils::test_command($test);
    $test->{'stdout'} =~ s/^\s*//gmx;
    my(undef, $metrics_url) = split(/\s+/, $test->{'stdout'});
    ok($metrics_url, "got metrics url: ".($metrics_url || "none"));
    $metrics_url =~ s|###SITE###|$site|g;
    TestUtils::test_url({ url => 'http://127.0.0.1:8003'.$metrics_url, like => ['/process_open_fds/', '/grafana_stat_totals_dashboard/'] });
};
